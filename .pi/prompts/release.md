...
2. Read `.system/RULES.md`, `.system/issues/`, `CHANGELOG.md`, `README.md`, `mise.toml`.
...
5. Run `RELEASE_TAG=$ARGUMENTS mise run release --check ghcr.io/bugabinga/hallucygenie:$ARGUMENTS`.
6. Cut release with `mise run release $ARGUMENTS`.
7. `mise run release` must open the exact release image in Chrome, wait for the tester to close Chrome or press Ctrl+C, ask `Manual test OK? [y/N]`, then tag only on `y`.
...
```sh
RELEASE_TAG=$ARGUMENTS mise run release --check ghcr.io/bugabinga/hallucygenie:$ARGUMENTS
...
- Never tag unless `mise run release` opened the exact image in Chrome and the tester answered `y`.
