...
test("send message shows user bubble and assistant response", async ({ page }) => {
  ...
  await page.waitForSelector(".message--assistant .message-content:not(:empty)", {
    timeout: 30000,
  }).catch(() => {
...
test("page reload restores chat history", async ({ page }) => {
  ...
  await expect(page.locator(".message--user .message-content").last()).toHaveText("Remember this!");
