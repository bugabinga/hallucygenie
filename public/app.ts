...
// Phase 1: extract fenced code blocks so they bypass all transforms
function extractCodeBlocks(text: string, codeStore: string[]): string {
    return text.replace(/```([\s\S]*?)```/g, (_m, content) => {
        const lines = content.split("\n");
        const lang = lines[0]?.trim() || "";
        const code = lang ? lines.slice(1).join("\n") : content;
        const idx = codeStore.length;
        codeStore.push(
            `<pre><code${lang ? ` class="lang-${lang}"` : ""}>${escapeHtml(code.trimEnd())}</code></pre>`,
        );
        return `\x00CODE${idx}\x00`;
    });
}

// Phase 2: extract inline code so it bypasses transforms
function extractInlineCode(html: string, codeStore: string[]): string {
...
function closeOpenBlocks(result: string[], s: BlockState): void {
    if (s.inList) {
        result.push(s.listType === "ul" ? "</ul>" : "</ol>");
        s.inList = false;
    }
    if (s.inBlockquote) {
...
function renderHr(result: string[], state: BlockState): void {
    closeOpenBlocks(result, state);
    result.push("<hr>");
}

function renderHeading(line: string, result: string[], state: BlockState): void {
    const m = line.match(/^(#{1,6})\s+(.+)$/);
    if (!m) return;
    closeOpenBlocks(result, state);
    result.push(`<h${m[1].length}>${inlineMarkdown(m[2])}</h${m[1].length}>`);
}
...
function renderTableRow(
    ...
    // Detect separator rows (---|---) inside a table
...
function renderBlockquoteLine(line: string, result: string[], state: BlockState): void {
    ...
    result.push(`<p>${inlineMarkdown(m[1])}</p>`);
}

function renderUlItem(line: string, result: string[], state: BlockState): boolean {
    ...
    result.push(`<li>${inlineMarkdown(m[1])}</li>`);
    return true;
}

function renderOlItem(line: string, result: string[], state: BlockState): boolean {
...
function renderParagraph(line: string, result: string[]): void {
    if (line.trim() === "") {
        result.push("");
    } else {
...
// Phase 3: render block-level elements from lines
function renderBlockLines(lines: string[]): string[] {
    const result: string[] = [];
    const state: BlockState = { inList: false, listType: "", inBlockquote: false, inTable: false };

    for (let i = 0; i < lines.length; i++) {
        const line = lines[i];

        // Code placeholder lines pass through untouched
        ...
        if (/^(\*{3,}|-{3,}|_{3,})\s*$/.test(line.trim())) {
            renderHr(result, state);
            continue;
        }

        // Headings
        if (/^#{1,6}\s+/.test(line)) {
            renderHeading(line, result, state);
            continue;
        }
        ...
        if (trimmed.startsWith("|") && trimmed.endsWith("|")) {
            if (!state.inTable) {
                ...
                i = renderTableRow(line, lines, i, result, state);
            }
            continue;
        }
        if (state.inTable) {
        ...
        // Blockquote
        if (/^&gt;\s?/.test(line)) {
            renderBlockquoteLine(line, result, state);
            continue;
        }
        if (state.inBlockquote) {
            result.push("</blockquote>");
        ...
        if (renderUlItem(line, result, state)) continue;
        // List items — ordered
        if (renderOlItem(line, result, state)) continue;

        // Non-list line closes list
        if (state.inList) {
            result.push(state.listType === "ul" ? "</ul>" : "</ol>");
            state.inList = false;
        }

        // Paragraph or blank line
        renderParagraph(line, result);
    }
...
// Main entry point
// Phase 1: extract code blocks → phase 2: extract inline code →
// phase 3: escape HTML → phase 4: block transforms →
// phase 5: restore code → phase 6: clean empty <p>
export function renderMarkdown(text: string): string {
    const codeStore: string[] = [];

    let html = extractCodeBlocks(text, codeStore);
    html = extractInlineCode(html, codeStore);
    html = escapeHtml(html);

    const lines = html.split("\n");
    const result = renderBlockLines(lines);

    let output = result.join("\n");
    output = output.replace(/\x00CODE(\d+)\x00/g, (_m, idx) => codeStore[parseInt(idx)]);
